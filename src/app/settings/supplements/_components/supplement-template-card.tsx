...
export function SupplementTemplateCard({ template }: { template: SupplementTemplate }) {
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      const result = await updateSupplementTemplateAction(template.id, formData);
      if (result?.errors) {
      ...
      <div className="flex flex-col gap-2 rounded-md border border-border bg-background p-3 text-sm">
        ...
        <div className="flex gap-2">
          ...
          <select
            value={doseUnit}
            onChange={(event) =>
              setDoseUnit(event.target.value as (typeof supplementDoseUnitEnum.enumValues)[number])
            }
            className="rounded-md border border-border bg-card px-2 py-1 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/30"
          >
          ...
          <select
            ...
            onChange={(event) =>
              setFrequency(
                event.target.value as (typeof supplementFrequencyEnum.enumValues)[number],
              )
            }
